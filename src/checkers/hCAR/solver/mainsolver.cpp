...
namespace car
{
	 ...
	 * @param unroll_level 
	 */
	MainSolver::MainSolver (Model* m, const bool verbose, int unroll_level) 
	{
    ...
    void MainSolver::set_assumption(Osequence* O, State*s,  const int frame_level, const bool forward)
    {
		assumption_.clear();
		if (frame_level > -1)
			assumption_.push (SAT_lit (flag_of(O,frame_level)));
	...
	Cube MainSolver::get_conflict_no_bad (const int bad)
	{
		...
		sort(res.begin(),res.end(),car::comp);
	...
	void MainSolver::enable_level(int level)
	{
		...
		for(int i = 2; i <= level; ++i)
		{
			...
			assumption_.push (SAT_lit (flag_for_this_level));
		}
	}
